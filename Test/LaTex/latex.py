...
class Latex:
    # Initialization
    def __init__(self, filename, chunk_len, batch_size, gpu, model_saved, hidden_size, n_layers, learning_rate, print_every, n_epochs, prime_str, predict_len, temperature, net):
        ...
        # Create the RNN
        self.decoder = model.CharRNN(self.n_characters, hidden_size, self.n_characters, n_layers)
    ...
    def train(self):
        # Initialization
        hidden = self.decoder.init_hidden(self.batch_size, self.net)
        if self.gpu:
            hidden = hidden.cuda()
    ...
    def predict(self):
        ...
        hidden = self.decoder.init_hidden(1, self.net)
