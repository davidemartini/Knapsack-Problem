...
class Network(nn.Module): #inherinting from nn.Module
    ...
    def __init__(self, input_size, output_size, gpu): #[self,input neuroner, output neuroner]
        ...
        self.gpu = gpu
        self.lhid1 = nn.Linear(self.input_size, 8192)
        self.lhid1a = nn.Linear(8192, 4096)
        self.lhid1b = nn.Linear(4096, 2048)
        self.lhid1c = nn.Linear(2048, 1024)
        self.lhid1d = nn.Linear(1024, 512)
    ...
    def forward(self, state):
        ...
        if self.gpu:
            ...
            level = self.lhid1a(level).cuda()
            level = self.lhid1b(level).cuda()
            level = self.lhid1c(level).cuda()
            level = self.lhid1d(level).cuda()
            ...
            level = self.lhid1a(level)
            level = self.lhid1b(level)
            level = self.lhid1c(level)
            level = self.lhid1d(level)
    ...
    def initHidden(self):
        hid = []
        if self.gpu:
            hid.append(torch.rand(self.input_size, 8192).cuda())
            hid.append(torch.rand(8192, 4096).cuda())
            hid.append(torch.rand(4096, 2048).cuda())
            hid.append(torch.rand(2048, 1024).cuda())
            hid.append(torch.rand(1024, 512).cuda())
            ...
            hid.append(torch.rand(self.input_size, 8192))
            hid.append(torch.rand(8192, 4096))
            hid.append(torch.rand(4096, 2048))
            hid.append(torch.rand(2048, 1024))
            hid.append(torch.rand(1024, 512))
